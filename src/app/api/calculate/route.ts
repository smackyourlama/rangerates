...
export async function POST(request: Request) {
  try {
    ...
    const originAddress = payload?.originAddress?.trim();

    if (!address) {
      return NextResponse.json({ error: "Please enter a delivery address." }, { status: 400 });
    }

    const quote = await buildDeliveryQuote(address, originAddress);
    ...
    const message = error instanceof Error ? error.message : "Unable to calculate delivery fee right now.";
