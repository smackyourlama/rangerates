...
export default function BillingPage() {
  ...
  return (
    <DashboardShell title="Billing" subtitle="Manage your plan and payment details.">
        ...
        <div className="space-y-6">
          ...
          <div className="grid gap-4 md:grid-cols-3">
            <div className="rounded-[24px] border border-white/80 bg-white/90 p-5 shadow-soft">
              ...
              <div className="mt-2 text-sm text-slate-600">{billingEnabled ? "Billing is available for this account." : "Billing is not available for this account yet."}</div>
            </div>
            <div className="rounded-[24px] border border-white/80 bg-white/90 p-5 shadow-soft">
            ...
            <div className="rounded-[24px] border border-white/80 bg-white/90 p-5 shadow-soft">
              ...
              <div className="mt-2 text-sm text-slate-600">{subscription?.stripeCustomerId ? "Payment details can be managed from the billing portal." : "The billing portal will be available after your first payment."}</div>
            </div>
          </div>

          <Panel title="Current subscription" description="Your current plan and renewal details.">
            {!subscription ? (
              <EmptyState title="No active subscription yet" description="Choose a plan below to get started." actionHref="#plans" actionLabel="View plans" />
            ) : (
          ...
          <Panel title="Subscription plans" description="Choose the plan that fits your team.">
              ...
              {plans.map((plan) => (
                <div key={plan.id} className="rounded-[24px] border border-slate-100 bg-white p-5 shadow-soft">
                  ...
                  <div className="mt-4 rounded-2xl bg-slate-50 p-4 text-sm leading-7 text-slate-600">
                    Select this plan to continue to checkout.
                  </div>
